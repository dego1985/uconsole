...
def load_csv():
    df = pd.read_csv(
        "https://docs.google.com/spreadsheets/d/1dPY1LQAMbR60w246kQV6Iwi4UqmZKgIa4iAlP3wppXo/export?format=csv&gid=995531617"
    )
    df = df.rename(
        columns={
    ...
    df["Date of Submit"] = pd.to_datetime(
        df["_submit"], format="%m/%d/%Y", errors="coerce"
    )

    df["Date of Notice"] = pd.to_datetime(
        df["_notice"], format="%m/%d/%Y", errors="coerce"
    )
    ...
    df["Order Detail"] = [
        to_order_pattern(x, y, z, w)
        for x, y, z, w in zip(
            df["model"], df["4G"] == "Yes", df["CM4"] == "Yes", df["color"]
        )
    ]
...
app.layout = html.Div(
    [
        html.H1(
            children="uConsole Order Progress", style={"textAlign": "center"}
        ),
        html.H3(
            children=html.A(
                "Link to Source forum",
                href="https://forum.clockworkpi.com/t/highest-order-number-that-received-the-shipment-notice/10478",
                target="_blank",
            ),
...
def update_graph(
    ...
    if np.sum(~valids) > 0:
        df["Order Detail"] = [
            x if valid else "_maybe error_"
            for x, valid in zip(df["Order Detail"], valids)
        ]
